...
const sendCareersMail = async (data) => {
    const { fullName, email, phone, position, resumeLink, message } = data;
    const safeFullName = (fullName || "Applicant").trim();
    const safeEmail = (email || "").trim();
    const safePhone = (phone || "Not provided").trim();
    const safePosition = (position || "Not specified").trim();
    const safeResumeLink = (resumeLink || "").trim();
    const safeMessage = (message || "No cover letter message provided.").trim();
    ...
    const mailOptions = {
        ...
        subject: `New Job Application: ${safePosition} - ${safeFullName}`,
        text: `New Job Application from ${safeFullName} (${safeEmail})\nPosition: ${safePosition}\nPhone: ${safePhone}\nResume Link: ${safeResumeLink}\n\nCover Letter / Message:\n${safeMessage}`,
        html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f6fc; padding: 40px 20px; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 30px rgba(0, 82, 255, 0.08); border: 1px solid #eef2f6;">
                ...
                <div style="background: linear-gradient(135deg, #050B14 0%, #0a192f 100%); color: #ffffff; padding: 30px; text-align: center; border-bottom: 3px solid #0052FF;">
                    <h2 style="margin: 0; font-size: 24px; font-weight: 800; letter-spacing: 1px; color: #ffffff;">EMSTRAP</h2>
                    <p style="margin: 5px 0 0 0; font-size: 14px; color: #00E5FF; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">New Job Application</p>
                </div>
                
                <div style="padding: 30px 40px;">
                    <!-- Message Summary -->
                    <p style="font-size: 15px; line-height: 1.6; color: #475569; margin-top: 0;">
                        A candidate has submitted a job application through the EMSTRAP Careers page. Below are their details:
                    </p>
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin: 25px 0;">
                        ...
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            <td style="padding: 12px 0; font-size: 14px; font-weight: 600; color: #64748b;">Position Applied</td>
                            <td style="padding: 12px 0; font-size: 14px; font-weight: 700; color: #0052FF;">${safePosition}</td>
                        </tr>
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            <td style="padding: 12px 0; font-size: 14px; font-weight: 600; color: #64748b;">Email Address</td>
                            <td style="padding: 12px 0; font-size: 14px; font-weight: 600; color: #0052FF;"><a href="mailto:${safeEmail}" style="color: #0052FF; text-decoration: none;">${safeEmail}</a></td>
                        ...
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            <td style="padding: 12px 0; font-size: 14px; font-weight: 600; color: #64748b;">Resume Link</td>
                            <td style="padding: 12px 0; font-size: 14px; font-weight: 700; color: #00E5FF;"><a href="${safeResumeLink}" target="_blank" style="color: #00E5FF; text-decoration: underline;">View Resume / CV</a></td>
                        </tr>
                    </table>
                    
                    <!-- Cover Letter -->
                    <h3 style="font-size: 16px; font-weight: 700; color: #0f172a; margin: 25px 0 10px 0; border-left: 4px solid #0052FF; padding-left: 10px;">Cover Letter / Message</h3>
                    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 12px; font-size: 14px; line-height: 1.6; color: #334155; white-space: pre-wrap; font-style: italic;">
                        "${safeMessage}"
                    </div>
                    ...
                    <!-- Action Button -->
                    <div style="text-align: center; margin: 35px 0 15px 0;">
                        <a href="mailto:${safeEmail}?subject=Re: EMSTRAP Job Application - ${safePosition}" style="background-color: #0052FF; color: #ffffff; text-decoration: none; padding: 14px 30px; font-size: 14px; font-weight: 700; border-radius: 8px; display: inline-block; box-shadow: 0 4px 12px rgba(0, 82, 255, 0.2);">
                ...
                <!-- Footer -->
                <div style="background-color: #f8fafc; padding: 20px; text-align: center; border-top: 1px solid #e2e8f0; font-size: 12px; color: #94a3b8;">
                    This application was generated automatically from the EMSTRAP corporate careers page.
                </div>
            </div>
        </div>
        `
    ...
    try {
        ...
        console.warn("⚠️ SMTP Careers Send Failed.");
        console.warn("Error Details:", error.message);
        throw error;
    }
};
